...
public class CalculatorController {
    ...
    public List<CalculationDto> search(
        ...
        Specification<CalculationEntity> spec = (root, query, cb) ->
                cb.between(root.get("createdAt"), from, to);

        if (operation != null && !operation.isBlank()) {
            spec = spec.and((root, query, cb) ->
                    cb.equal(root.get("operation"), operation.trim().toUpperCase()));
        }
        if (leftRadix != null && !leftRadix.isBlank()) {
            spec = spec.and((root, query, cb) ->
                    cb.equal(root.get("leftRadix"), leftRadix.trim().toUpperCase()));
        }
        if (rightRadix != null && !rightRadix.isBlank()) {
            spec = spec.and((root, query, cb) ->
                    cb.equal(root.get("rightRadix"), rightRadix.trim().toUpperCase()));
        }
